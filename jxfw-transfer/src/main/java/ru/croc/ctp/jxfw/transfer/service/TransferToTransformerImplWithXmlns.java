...
import static ru.croc.ctp.jxfw.transfer.impl.BinaryPropertyService.HeaderName;
...
@Component
@Profile("exportXml")
public class TransferToTransformerImplWithXmlns implements TransferToTransformer {
    ...
    public String toXml(@Nonnull DomainTo dto) {
        final StringBuilder xmlBuilder = new StringBuilder(
                "<" + dto.getType() + " xmlns:dt=\"urn:schemas-microsoft-com:datatypes\""
                + " oid=\"" + dto.getId() + "\" >");
        ...
        dto.forEachProperty((name, val) -> {
            if (val != null) {
                if (isDomainType(dto, name)) {
                    ...
                    if (binaryPropertyService.isBlob(dto, name)) {
                        final Map<String, Object> binaryProps = (Map) val;
                        xmlBuilder.append("<").append(name).append(" ")
                                .append("dt:dt=\"bin.base64\"").append(" ")
                                .append("data-size=\"")
                                .append(binaryProps.getOrDefault(HeaderName.FILE_SIZE, 0))
                                .append("\">")
                                .append(binaryProps.getOrDefault(HeaderName.VALUE, ""))
