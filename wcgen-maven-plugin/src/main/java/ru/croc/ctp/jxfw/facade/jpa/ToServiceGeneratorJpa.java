 ...
 */
public class ToServiceGeneratorJpa<K extends TypeName> extends ToServiceGenerator<K> {
     ...
     */
    public ToServiceGeneratorJpa(XFWClass clazz, Set<XFWClass> xfwClasses, String storageType, K keyType) {
        super(clazz, xfwClasses, storageType, keyType);
    }

    @Override
    protected void addClassLevelAnnotations() {
        super.addClassLevelAnnotations();
    ...
    protected boolean overrideCreateDummyDomainObject(CodeBlock.Builder codeBlock, ClassName fieldType) {        
        codeBlock.add(CodeBlock
                .builder()
                .beginControlFlow("if (props.length > 1)")
                .addStatement("return domainToServicesResolver.<$1T, $2T>resolveToService($3S).parsePropValue(props[props.length - 1], v)",
                        get(fieldType.packageName() + ".facade.webclient", fieldType.simpleName() + "ToService"), fieldType, fieldType.simpleName())
    ...
    protected NavigableStructureInfo buildNavigableStructureInfo(XFWReference xfwReference) {
        if (markedAsJpaTransient(xfwReference)) {
            ...
            return new NavigableStructureInfo(eAnnotation, containerType, xfwReference.getName(), fieldType, null, true,
                    true);
        }
